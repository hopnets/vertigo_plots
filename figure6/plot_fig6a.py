...
for i in range(len(data)):
    plots.append(plt.plot(ind[:len(data_m[i])], data_m[i], color=data[i][2], marker=data[i][3],
                linewidth=5, markevery=1, markersize=16, label=data[i][1])[0])
...
plt.yticks([0.001, 0.01, 0.1, 1, 5], ["1ms", "10ms", "100ms", "1s", "5s"], **font_label)
ax.grid(axis='y', which='both')
# ax.set_yticks([0,1,2,3,4])

if config["legend"] == True:
    plt.legend(plots, labels,loc='upper left', bbox_to_anchor=(0, 1.85), ncol=2)
